    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-float max-h-[86vh] max-w-3xl overflow-y-auto rounded-3xl p-0 sm:max-w-3xl">
        <DialogHeader className="border-b border-border/60 bg-secondary/20 px-6 py-5 pr-14">
          ...
          <DialogDescription>Profile có Library và dữ liệu ứng dụng riêng. Các profile dùng cùng một MosaicMedia sẽ dùng chung toàn bộ media.</DialogDescription>
        </DialogHeader>
        <div className="space-y-5 px-6 py-5">
          <section className="rounded-2xl border border-border/65 bg-secondary/25 p-3">
            ...
            <div className="space-y-1.5"><div className="flex gap-2"><input value={newName} onChange={(event) => updateNewName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void create(); }} placeholder="Tên profile" aria-invalid={Boolean(newNameError)} className={`${inputClass} ${newNameError ? "border-destructive/70 focus:border-destructive focus:ring-destructive/25" : ""}`} /><button onClick={() => void create()} disabled={!newName.trim() || working === "create"} className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-primary/90 px-3.5 text-sm font-medium text-primary-foreground disabled:opacity-45"><Plus className="size-4" />Tạo</button></div>{newNameError && <p role="alert" className="px-1 text-xs text-destructive">{newNameError}</p>}</div>
          </section>
