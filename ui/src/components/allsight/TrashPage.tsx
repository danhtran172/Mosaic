...
function Preview({ media }: { media: MediaItem }) {
  ...
  return media.url ? <img src={media.url} alt="" className="size-full object-contain" /> : <span className="grid size-full place-items-center bg-muted text-xs text-muted-foreground">Đang tải…</span>;
}
...
export function TrashPage({ onBack }: { onBack: () => void }) {
  ...
  return <div className="flex h-screen flex-col overflow-hidden bg-background"><div className="window-titlebar flex h-[18px] shrink-0 items-center border-b border-border/55"><span className="pl-3 text-[11px] font-medium tracking-wide text-muted-foreground">InDeck</span></div><main className="app-scroll flex-1 overflow-y-auto p-6"><header className="mb-6 flex items-center gap-3"><button onClick={onBack} className="glass-btn grid size-9 place-items-center rounded-lg"><ArrowLeft className="size-4" /></button><div><h1 className="font-display text-xl font-semibold">Thùng rác</h1><p className="text-sm text-muted-foreground">Khôi phục hoặc xóa vĩnh viễn các mục InDeck không còn quản lý.</p></div>{selected.length > 0 && <div className="ml-auto flex gap-2"><button onClick={restoreSelected} className="glass-btn flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm"><RotateCcw className="size-4" /> Khôi phục</button><button onClick={purgeSelected} className="flex items-center gap-1.5 rounded-lg border border-destructive/30 px-3 py-2 text-sm text-destructive"><Trash2 className="size-4" /> Xóa vĩnh viễn</button></div>}</header>{state.trashFolders.length === 0 && hidden.length === 0 && galleryTrash.length === 0 ? <div className="grid min-h-72 place-items-center rounded-2xl border border-dashed border-border text-sm text-muted-foreground">Thùng rác trống.</div> : <div className="space-y-7">{state.trashFolders.length > 0 && <section><h2 className="mb-3 text-xs font-semibold tracking-wider text-muted-foreground uppercase">Gallery đã xóa</h2><div className="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-3">{state.trashFolders.map((folder) => <button key={folder.id} onClick={(event) => select(event, `folder:${folder.id}`)} className={cn("overflow-hidden rounded-xl border border-border/65 bg-secondary/20 text-left", selected.includes(`folder:${folder.id}`) && "border-primary ring-1 ring-primary")}><span className="grid aspect-[4/3] place-items-center bg-muted"><Folder className="size-8 text-muted-foreground" /></span><p className="truncate px-3 py-2 text-sm font-medium">{folder.name}</p></button>)}</div></section>}{hidden.length > 0 && <section><h2 className="mb-3 text-xs font-semibold tracking-wider text-muted-foreground uppercase">Ảnh đã ẩn</h2><div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-3">{hidden.map((media) => card(`media:${media.id}`, media))}</div></section>}{galleryTrash.length > 0 && <section><h2 className="mb-3 text-xs font-semibold tracking-wider text-muted-foreground uppercase">Ảnh đã xóa khỏi Gallery</h2><div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-3">{galleryTrash.map(({ gallery, media }) => card(`gallery-media:${gallery.id}:${media.id}`, media, gallery.name))}</div></section>}</div>}</main></div>;
}
